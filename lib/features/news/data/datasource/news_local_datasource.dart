...
abstract class NewsLocalDatasource {
  ...
  Future<List<NewsModel>> retrieveNews() =>
      throw UnimplementedError('Stub');
}
...
class NewsLocalDataSourceImpl implements NewsLocalDatasource {
  ...
  Future<List<NewsModel>> retrieveNews() async {
    try {
      ...
      List<NewsModel> news = newsJsonString.map((news) =>
          NewsModel.fromJson(jsonDecode(news))).toList();
      return news;
    } catch (e, stackTrace) {
      debugPrint('NewsLocalDataSource [retrieveNews] exception: ${e.toString()}');
  ...
  Future<void> saveNews(List<NewsModel> news) async {
    try {
      ...
      List<String> newsJsonList = news.map((n) => jsonEncode(n.toJson())).toList();
